...
describe('EXPERIMENT-2: Step 4 - Extraction', () => {
    it('should extract individual assets based on coordinates', async () => {
        const currentDir = __dirname;
        const spritePath = path.join(currentDir, "sprites.png");
        const coordinatesPath = path.join(currentDir, "coordinates.json");
        ...
        if (!fs.existsSync(coordinatesPath)) throw new Error("coordinates.json not found. Run Step 3.");

        if (!fs.existsSync(extractedDir)) fs.mkdirSync(extractedDir, { recursive: true });

        const spriteBuffer = fs.readFileSync(spritePath);
        const detectedItems = JSON.parse(fs.readFileSync(coordinatesPath, 'utf-8'));
